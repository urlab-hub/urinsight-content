...
import { slidesFor, renderHtml, type InsightImage, type ContentLayout } from './template.js';
...
export async function generate(contentFile: string, options: { outputRoot?: string; date?: string; provider?: CoverImageProvider; insightImage?: InsightImage; contentLayout?: ContentLayout } = {}) {
  ...
  try {
    ...
    for (const [index, slide] of slides.entries()) {
      ...
      await page.setContent(renderHtml(content, slide, font, image, options.insightImage, options.contentLayout), { waitUntil: 'load' });
      let report;
