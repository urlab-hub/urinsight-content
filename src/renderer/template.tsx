...
import { tokens as t } from '../config/tokens.js';
...
export function renderHtml(content: CarouselContent, slide: Slide, fontBase64: string, image?: string): string {
  const css = `
    ...
    *{box-sizing:border-box}html,body{margin:0;width:${t.canvas.width}px;height:${t.canvas.height}px}body{font-family:${t.font.family};font-weight:${t.font.regular};font-synthesis:none;-webkit-font-smoothing:antialiased}
    .slide{position:relative;width:${t.canvas.width}px;height:${t.canvas.height}px;background:${t.colors.paper};color:${t.colors.ink};--accent:${t.categories[content.category].color}}
    h1,h2,p{margin:0}h1,h2,.key,.summary-key,.slogan{font-weight:${t.font.bold}}
    mark{position:relative;background:transparent;color:white;padding:0;white-space:nowrap;isolation:isolate}
    mark::before{content:'';position:absolute;left:var(--ink-left,0px);top:calc(var(--ink-top,0px) - var(--highlight-top,0px));width:var(--ink-width,100%);height:calc(var(--ink-height,100%) + var(--highlight-top,0px) + var(--highlight-bottom,0px));background:var(--accent);z-index:-1}
